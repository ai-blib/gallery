...
export const MyOrders = memo(withRouter(({history}: Props) => {
    const [selectedToken, setSelectedToken] = useState<any>({});
    const [visible, setVisible] = useState<boolean>(false);
    const {myOrders} = getProfileInfo();
    ...
    return (
        <Styled.Wrap>
            ...
            <Styled.Content>
                {active === 0 && <Styled.CardsWrap active={active === 0}>
                    {(((artList) ? artList : itemList).map(
